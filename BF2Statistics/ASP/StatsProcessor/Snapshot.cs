    ...
    class Snapshot : GameResult
    {
        ...
        public Snapshot(string Snapshot, DateTime Date, StatsDatabase Database)
        {
            ...
            try
            {
                ...
                for (int i = 2; i < Data.Length; i += 2)
                {
                    ...
                    else if (Parts[0] == "pID")
                    {
                        ...
                        KillData.Add(Data[i + 1], Data[i + 3]);
                    else
                        PlayerData.Add(Parts[0], Data[i + 1]);
                }
            }
            ...
            try
            {
                ...
                this.RoundStart = (int)Convert.ToDouble(StandardData["mapstart"], CultureInfo.InvariantCulture.NumberFormat);
                this.RoundEnd = (int)Convert.ToDouble(StandardData["mapend"], CultureInfo.InvariantCulture.NumberFormat);
                this.RoundTime = TimeSpan.FromSeconds(this.RoundEnd - this.RoundStart);

                // Misc Data
                this.GameMode = Int32.Parse(StandardData["gm"]);
                this.Mod = StandardData["v"];
            ...
            if (this.MapId == 99)
            {
                ...
                Rows = Driver.Query("SELECT id FROM mapinfo WHERE name=@P0", this.MapName);
                if (Rows.Count == 0)
                {
                    ...
                    this.MapId = Int32.Parse(Rows[0]["id"].ToString()) + 1;

                    // Insert map data, so we dont lose this mapid we generated
                    Driver.Execute("INSERT INTO mapinfo(id, name) VALUES (@P0, @P1)", this.MapId, this.MapName);
                }
                else
                    this.MapId = Int32.Parse(Rows[0]["id"].ToString());
            }
            else
                this.IsCustomMap = (MapId >= 700);

            // Set whether or not this data is already been processed
            Rows = Driver.Query(
                "SELECT COUNT(mapid) AS count FROM round_history WHERE timestamp = @P0 AND mapid = @P1",
                this.RoundStart, this.MapId
            );
            this.IsProcessed = (Int32.Parse(Rows[0]["count"].ToString()) > 0);
        }
        ...
        public void ProcessData()
        {
            ...
            if (this.IsCustomMap)
                Log(String.Format("Custom Map ({0})...", this.MapId), LogLevel.Notice);
            else
                Log(String.Format("Standard Map ({0})...", this.MapId), LogLevel.Notice);
            ...
            try
            {
                ...
                foreach (Player Player in this.Players)
                {
                    ...
                    // Fetch the player
                    Rows = Driver.Query("SELECT COUNT(id) AS count FROM player WHERE id=@P0", Player.Pid);
                    if (int.Parse(Rows[0]["count"].ToString()) == 0)
                    {
                        ...
                        InsertQuery.SetField("joined", this.TimeStamp);
                        InsertQuery.SetField("rndscore", Player.RoundScore);
                        InsertQuery.SetField("lastonline", RoundEnd);
                    ...
                    else
                    {
                        ...
                        if (MainForm.Config.ASP_StatsRankCheck && DbRank > Player.Rank)
                        {
                        ...
                        UpdateQuery.SetField("lastonline", this.TimeStamp, ValueMode.Set);
                    ...
                    Driver.Execute(
                        ...
                        Player.Pid, this.TimeStamp, Player.RoundTime, Player.RoundScore, Player.CommandScore, 
                    ...
                    if (Rows.Count == 0)
                    {
                        ...
                        if (Player.ArmyId < 14)
                        {
                            InsertQuery.SetField("win" + Player.ArmyId, ((OnWinningTeam) ? 1 : 0));
                            InsertQuery.SetField("loss" + Player.ArmyId, ((!OnWinningTeam) ? 1 : 0));
                    ...
                    foreach (KeyValuePair<string, string> Kill in Player.Victims)
                    {
                        string Victim = Kill.Key;
                        int KillCount = Int32.Parse(Kill.Value);
                        Rows = Driver.Query("SELECT count FROM kills WHERE attacker=@P0 AND victim=@P1", Player.Pid, Victim);
                        if (Rows.Count == 0)
                        {
                    ...
                    Rows = Driver.Query("SELECT time0 FROM kits WHERE id=" + Player.Pid);
                    if (Rows.Count == 0)
                    {
                    ...
                    Rows = Driver.Query("SELECT time0 FROM vehicles WHERE id=" + Player.Pid);
                    if (Rows.Count == 0)
                    {
                    ...
                    Rows = Driver.Query("SELECT time0 FROM weapons WHERE id=" + Player.Pid);
                    if (Rows.Count == 0)
                    {
                        // Prepare Query
                    ...
                    if (Player.CompletedRound || !MainForm.Config.ASP_AwardsReqComplete)
                    {
                        ...
                        foreach (KeyValuePair<int, int> Award in Player.EarnedAwards)
                        {
                            ...
                            if (Convert.ToInt32(Driver.ExecuteScalar(Query, Player.Pid, Award.Key)) == 0)
                            {
                                ...
                                if (IsBadge)
                                {
                                    ...
                                    for (int j = 1; j < Award.Value; j++)
                                    {
                                        ...
                                        if (Convert.ToInt32(Driver.ExecuteScalar(Query, Player.Pid, Award.Key, j)) == 0)
                                        {
                                            ...
                                            InsertQuery.SetField("earned", (this.TimeStamp - 5) + j);
                                ...
                                InsertQuery.SetField("earned", this.TimeStamp);
                                InsertQuery.SetField("first", ((IsMedal) ? this.TimeStamp : 0));
                                InsertQuery.Execute();
                            ...
                            else // === Player has recived this award prior === //
                            {
                                ...
                                if (IsMedal)
                                {
                                    ...
                                    UpdateQuery.SetField("earned", this.TimeStamp, ValueMode.Set);
                                    UpdateQuery.Execute();
                                }
                            }
                ...
                Rows = Driver.Query("SELECT COUNT(id) AS count FROM mapinfo WHERE id=" + this.MapId);
                ...
                InsertQuery.SetField("timestamp", this.RoundStart);
                InsertQuery.SetField("mapid", this.MapId);
                InsertQuery.SetField("time", this.RoundTime.Seconds);
        ...
        private void SmocCheck()
        {
            ...
            if (Players.Count == 1)
            {
                ...
                if (Rows.Count > 0)
                {
                    ...
                    if (Id != Sid && (Earned + MinTenure) < this.TimeStamp)
                    {
                        ...
                        Driver.Execute("INSERT INTO awards(id,awd,earned) VALUES(@P0,@P1,@P2)", Id, 6666666, this.TimeStamp);
            ...
            if (Players.Count == 1)
            {
                ...
                if (Rows.Count > 0)
                {
                    ...
                    if (Id != Sid && (Earned + MinTenure) < this.TimeStamp)
                    {
                        ...
                        Driver.Execute("INSERT INTO awards(id,awd,earned) VALUES(@P0,@P1,@P2)", Id, 6666667, this.TimeStamp);
