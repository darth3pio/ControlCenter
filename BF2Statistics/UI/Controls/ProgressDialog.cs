...
namespace BF2Statistics 
{
	...
	public class ProgressDialog : Component 
    {
		...
		[DefaultValue(true)]
        [Category("Behaviour")]
        [Description("whether the progress dialog is automatically closed when the Value property equals or exceeds the value of Maximum.")]
        ...
        [Category("Appearance")]
        [Description("Indicates the title displayed on the progress dialog.")]
		public string Title 
        {
			get { return _title; }
			set 
        ...
        [Category("Appearance")]
        [Description("Indicates the message to be displayed when the user clicks the cancel button on the progress dialog.")]
		...
		[DefaultValue(false)]
        [Category("Appearance")]
        [Description("Indicates whether to have path strings compacted if they are too large to fit on a line.")]
		public bool CompactPaths 
        {
			get { return _compactPaths; }
			set 
            {
		...
		[DefaultValue(100)]
        [Category("Behaviour")]
        [Description("Indicates what the Value property will be equal to when the operation has completed.")]
		public int Maximum 
        {
			get { return _maximum; }
			set 
		...
		public int Value 
        {
			...
			set 
            {
                ...
                {
					UpdateProgress();
					if (_autoClose && (_value >= _maximum)) 
                        Close();
				}
			}
		}
		/// <summary>
		/// Gets or sets whether the progress dialog box will be modal to the parent window. 
        /// By default, a progress dialog box is modeless.
		/// </summary>
		[DefaultValue(false)]
        [Category("Behaviour")]
        [Description("Indicates whether the progress dialog box will be modal to the parent window. By default, a progress dialog box is modeless.")]
		public bool Modal 
        {
			get { return (_flags & PROGDLG.Modal) == PROGDLG.Modal; }
			set 
            {
				if (value)
		...
		[DefaultValue(true)]
        [Category("Behaviour")]
        [Description("Automatically estimate the remaining time and display the estimate on line 3.")]
		public bool ShowTimeRemaining 
        {
			get { return (_flags & PROGDLG.AutoTime) == PROGDLG.AutoTime; }
			set 
            {
				if (value) 
                {
		...
		[DefaultValue(true)]
        [Category("Appearance")]
        [Description("Display a minimize button on the dialog box's caption bar.")]
		public bool MinimizeButton 
        {
			get { return (_flags & PROGDLG.NoMinimize) != PROGDLG.NoMinimize; }
			set 
            {
				if (value)
		...
		[DefaultValue(true)]
        [Category("Appearance")]
        [Description("Display a progress bar on the dialog box.")]
		public bool ProgressBar 
        {
			get { return (_flags & PROGDLG.NoProgressBar) != PROGDLG.NoProgressBar; }
			set
            {
				if (value)
		...
		[DefaultValue(true)]
        [Category("Behaviour")]
        [Description("Indicates whether the operation can be cancelled. You should always show a cancel button unless absolutely necessary.")]
		public bool CancelButton 
        {
			get { return (_flags & PROGDLG.NoCancel) != PROGDLG.NoCancel; }
			set 
            {
				if (value)
                {
				...
				else 
                {
					if (Environment.OSVersion.Version.Major < 6) 
                        throw new NotSupportedException("This option is only available on Windows Vista or greater.");
        ...
        /// similar to a marquee display. Use this when you wish to indicate that progress is being made, 
        /// but the time required for the operation is unknown.
		/// </summary>
		[DefaultValue(false)]
        [Category("Behaviour")]
        [Description("Sets the progress bar to marquee mode.")]
		public bool Marquee 
        {
			get { return (_flags & PROGDLG.MarqueeProgress) == PROGDLG.MarqueeProgress; }
			set
            {
				if (value) 
                {
		...
		public bool HasUserCancelled
        {
			get 
            {
				return (_nativeProgressDialog != null) ? _nativeProgressDialog.HasUserCancelled() : false;
			}
		}
		...
		public ProgressDialog(IContainer container) : this() 
        {
			container.Add(this);
		}
